...
const PREFIXO_CACHE_GARCOM = 'bar-da-ladeira-garcom-'
const EVENTO_RECONEXAO = 'pwa-garcom-reconectar'
...
export default function PWAManagerGarcom() {
  ...
  const obterVersaoDoCache = async () => {
    try {
      ...
      const cache = cacheNames.find((c) => c.startsWith(PREFIXO_CACHE_GARCOM))
      if (!cache) return null
      return cache.replace(PREFIXO_CACHE_GARCOM, '')
    } catch {
      return null
    }
  }
  ...
  const dispararEventoReconexao = useCallback(() => {
    window.dispatchEvent(new CustomEvent(EVENTO_RECONEXAO))
  }, [])
  ...
  useEffect(() => {
    ...
    const reloadFlag = sessionStorage.getItem('pwa_garcom_reloading')
    if (reloadFlag === 'true') {
      sessionStorage.removeItem('pwa_garcom_reloading')
      return
    }
  ...
  const registerServiceWorker = async () => {
    try {
      const reg = await navigator.serviceWorker.register('/sw-garcom.js', {
        scope: '/garcom/',
        updateViaCache: 'none',
      })
      setRegistration(reg)
      ...
      const handleControllerChange = () => {
        ...
        isReloading = true
        sessionStorage.setItem('pwa_garcom_reloading', 'true')
      ...
      console.error('[PWA Garçom] Erro ao registrar Service Worker:', error)
    }
  }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      console.error('[PWA Garçom] Erro ao atualizar:', error)
      setIsUpdating(false)
      setUpdateAvailable(false)
    }
  }
  ...
  const clearCache = async () => {
    ...
    try {
      ...
      const cacheNames = await caches.keys()
      await Promise.all(
        cacheNames.filter((n) => n.includes('garcom')).map((n) => caches.delete(n))
      )
      if (registration?.active) {
      ...
      console.error('[PWA Garçom] Erro ao limpar cache:', error)
  ...
  if (estaOffline) {
    return (
      <div className="fixed inset-x-0 top-0 z-[10002] flex items-center justify-center gap-2 bg-destructive px-4 py-2 text-sm font-medium text-destructive-foreground shadow-md">
  ...
  return (
    <div
      className="fixed bottom-4 left-4 right-4 z-[10001] sm:left-auto sm:right-4 sm:w-full sm:max-w-sm animate-slide-up"
      role="status"
      aria-live="polite"
    >
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold tracking-tight text-foreground">
              {isUpdating ? 'Atualizando...' : isClearingCache ? 'Limpando cache...' : 'Atualização disponível'}
            </p>
